defmodule InvestorUploadApplication.InvestorsTest do
  ...
  describe "investors" do
    ...
    test "create_investor/1 with valid data creates a investor" do
      valid_attrs = %{state: "some state", first_name: "some first_name", last_name: "some last_name", ssn: "some ssn", date_of_birth: ~D[2025-07-15], phone_number: "some phone_number", street_address: "some street_address", city: "some city", street_address2: "some street_address2", zip_code: "some zip_code", csv_file: "some csv_file"}
      ...
      assert investor.ssn == "some ssn"
      assert investor.date_of_birth == ~D[2025-07-15]
      assert investor.phone_number == "some phone_number"
      assert investor.street_address == "some street_address"
      assert investor.city == "some city"
      assert investor.street_address2 == "some street_address2"
      assert investor.zip_code == "some zip_code"
      assert investor.csv_file == "some csv_file"
    end
    ...
    test "update_investor/2 with valid data updates the investor" do
      ...
      assert investor.ssn == "some updated ssn"
      assert investor.date_of_birth == ~D[2025-07-16]
      assert investor.phone_number == "some updated phone_number"
      assert investor.street_address == "some updated street_address"
      assert investor.city == "some updated city"
      assert investor.street_address2 == "some updated street_address2"
      assert investor.zip_code == "some updated zip_code"
      assert investor.csv_file == "some updated csv_file"
    end
